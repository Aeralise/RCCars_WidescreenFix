...
- `FPS / Game Speed Fix` - The main problem with slow menus and overall game at 70+ fps now is cured - you can set any fps via config file (500+ not recomended). This feature overrides in-game FPS Limit setting.
...
- `HUD Stretching fix` - major of the UI elements are stretching, but some other (in Split-Screen mode) renders in correct aspect and size. 16:9 is done for HUD during race, other aspect ratios is WIP.
...
- `Full game resolution` - both in-menu and in-race resolution should be set in one place in .ini file
...
2. Open .ini file and set **Width and Height** of your display resolution, choose the aspect (16:9 by default) and FOV (193 is default for 16:9). In game settings, choose the same resolution you set in the .ini.
